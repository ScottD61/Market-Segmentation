...
daisy5 <- daisy(df5, metric = "gower", type = list(ordratio = c(1:35))) 
#Pam algorithm with 3 clusters 
pam(daisy5, 3, diss = TRUE, mediods = NULL)

#Option 2: CLARA function
#Exclude dissimilarity matrix
library(cluster)
client5.clara <- clara(Store5, 3, metric = "euclidean", stand = FALSE, samples = 55,
                       sampsize = (1000), rngR = FALSE, pamLike = TRUE)
#Get results
#Get number in each sample
client5.clara$sample
#Get medoids?
client5.clara$medoids
#Get each cluster with width and neighbor
client5.clara$silinfo
#Create table showing user.id in cluster - WRONG
client5table <- table(Store5$Email, client5.clara$cluster)
#Show results
client5table
#Coerce object to data.frame
clust5groups <- as.data.frame(client5table, row.names = NULL)
#Create .CSV file 
write.table(client5table, file <- "/Users/scdavis6/Desktop/ClaraClient5Solutions.csv")
